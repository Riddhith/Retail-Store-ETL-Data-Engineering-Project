...
from resources.dev.config import error_folder_path_local
...
#if the file is there then check if the same file is present in the staging area
#with status A.If so then dont delete and try to rerun
...
logger.info(f"List of all files present at my local directory after download {all_files}")
...
if error_files:
    for file_path in error_files:
        if os.path.exists(file_path):
            ...
            source_prefix = os.path.join(config.s3_source_directory, file_name)
            destination_prefix = os.path.join(config.s3_error_directory, file_name)
...
for data in correct_files:
    ...
    logger.info("******** Final Dataframe from source which will be going through processing ********")
...
logger.info(f"************* customer data written to local disk as {config.customer_data_mart_local_file} ************")
...
for root, dirs, files in os.walk(config.sales_team_data_mart_partitioned_local_file):
    for file in files:
        ...
        relative_file_path = os.path.relpath(str(local_file_path), config.sales_team_data_mart_partitioned_local_file)
...
logger.info("*********** calculate sales every month billed amount *************")
...
logger.info("************ Deleting sales team data mart partitioned from local *************")
delete_local_file(config.sales_team_data_mart_partitioned_local_file)
logger.info("************ Deleted sales team data mart partitioned from local **************")
